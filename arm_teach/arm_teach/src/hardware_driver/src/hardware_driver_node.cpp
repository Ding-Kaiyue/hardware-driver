...
void onCanFrameReceived(const canfd_frame& frame) {
    ...
    // 检查是否是Raytron电机反馈数据（0x0300 + 电机ID）
    if (g_protocol_converter && (actual_can_id & 0xFF00) == raytron::FEEDBACK_BASE_ID) {
        // 解码反馈数据
...
int main(int argc, char** argv)
{
    ...
    try
    {
        ...
        while (running)
        {
            ...
            {
                // 使用新协议：广播请求所有电机反馈
                // 一次请求，所有电机会返回各自的反馈（0x0301~0x0306）
                canfd_frame feedback_request = protocol_converter.encodeBroadcastRequestFeedback();

                // 发送反馈请求
                if (usb_manager->sendFrame(feedback_request)) {
                    std::cout << "[TX] 广播请求所有电机反馈 | "
                    ...
                    std::cerr << "[ERROR] Failed to send broadcast feedback request" << std::endl;
                }
